...
st.set_page_config(page_title="Job Intel Dashboard", page_icon="🎯", layout="wide")
init_db()
...
def _add_to_watchlist(company_name):
    with get_conn() as conn:
        conn.execute("INSERT OR IGNORE INTO watchlist_companies (company_name) VALUES (?)", (company_name,))

def _remove_from_watchlist(company_id):
    with get_conn() as conn:
...
def _render_job_card(job):
    ...
    with st.container():
        ...
        with st.expander("Details / Actions"):
            ...
            with col2:
                st.write("**My Status**")
                status_list = ["new", "reviewing", "saved", "excluded", "applied"]
                idx = status_list.index(user_status) if user_status in status_list else 0
                new_status = st.selectbox("Status", status_list, index=idx,
                    key=f"status_{job['canonical_id']}", label_visibility="collapsed")
                memo = st.text_area("Memo", value=job.get("memo") or "",
                    key=f"memo_{job['canonical_id']}", height=80)
...
def _render_job_list(min_score, change_filter, status_filter, company_search, show_closed):
    ...
    if not jobs:
        st.info("공고 데이터가 없습니다. 왼쪽 'Run Pipeline Now'를 눌러 수집을 시작하세요.")
        return
    for job in jobs:
...
def _render_stats():
    ...
    with get_conn() as conn:
        ...
        company_dist = conn.execute(
            "SELECT company_name, COUNT(*) as cnt FROM canonical_jobs WHERE status='active' GROUP BY company_name ORDER BY cnt DESC LIMIT 15"
        ).fetchall()
        score_dist = conn.execute(
            """SELECT CASE WHEN total_score>=70 THEN '💎 HOT (70+)'
                          WHEN total_score>=45 THEN '✅ GOOD (45-69)'
                          WHEN total_score>=20 THEN '🔍 MAYBE (20-44)'
                          ELSE '⬇️ SKIP (<20)' END as tier, COUNT(*) as cnt
               FROM job_scores GROUP BY tier ORDER BY cnt DESC"""
        ).fetchall()
...
with st.sidebar:
    ...
    change_options = st.multiselect("Change Status", ["new", "changed", "unchanged"], default=["new", "changed"])
    user_status_options = st.multiselect("My Status", ["new", "reviewing", "saved", "excluded", "applied"], default=["new", "reviewing", "saved"])
    ...
    if st.button("▶️ Run Pipeline Now", use_container_width=True):
        with st.spinner("Running pipeline..."):
            try:
                ...
                st.success(f"Done! New:{result['jobs_new']} Changed:{result['jobs_changed']} Closed:{result['jobs_closed']}")
                st.rerun()
